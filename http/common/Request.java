/*
Copyright 2009-2016 Wanglichao
...
import static myTest.http.util.Util.read;
import static myTest.http.util.Util.toBase64;
...
public abstract class Request<T extends Request> {
    ...
    public String text() {
        try {
            connect();
            return responseCode() >= 400 ? Util.read(connection.getErrorStream()) : read(connection.getInputStream());
        } catch (IOException e) {
            throw new HttpException("Failed URL: " + url, e);
        }finally {
            dispose();
